 ...
 */
function safeParseString($str, $start, $end, $decode = false) {
    if ($str === null || empty($str)) {
        return '';
    }
    $result = CurlX::ParseString($str, $start, $end, $decode);
...
if ($min_product === null) {
    throw new Exception("No available product with valid price found. Retry attempt number " . $retries . ". Please try again.");
            ...
            $localization = strip_tags(trim(safeParseString($request->headers->request["cookie"], 'localization=', ';')));
            $TOKEN = safeParseString($request->body, 'authenticity_token" value="', '"');
            ...
            $total = safeParseString($request->body, 'data-checkout-payment-due-target="', '"');
            $payment_gateway = safeParseString($request->body, 'payment_gateway_', '"');
            ...
            $id_sh = safeParseString($request->body, '"id":"', '"');
            ...
            if(substr_count($request->body, "grecaptcha.render")){
                ...
                $SITEKEY = safeParseString($request->body, 'sitekey: "', '"');
                $cursl = safeParseString($request->body, 'var recaptchaCallback = function() {', '//]]>');
                $s = safeParseString($cursl, "s: '", "'");
        ...
        $resp = safeParseString($request->body, '<p class="notice__text">', '</p>');
...
$TOKEN = safeParseString($request->body, 'authenticity_token" value="', '"');
$shipping_method = safeParseString($request->body, '<div class="radio-wrapper" data-shipping-method="', '"');
